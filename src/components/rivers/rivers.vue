<template>
	<div>
		...
		<div class="river-content" ref="river">
		...
		<mu-drawer class="list-alert" right :open="open" >
			...
			<mu-tabs class="padding-top rivers-tab" :value="activeTab" @change="handleTabChange">
			  	<mu-tab value="/rivers" title="一河一长"/>
			  	<mu-tab value="/rivers/document" title="一河一档"/>
			  	<mu-tab value="/rivers/strajy" title="一河一策"/>
			  	<mu-tab value="/rivers/build" title="一河一建"/>
			  	<mu-tab value="/rivers/management" title="一河一管"/>
			  	<mu-tab value="/rivers/view" title="一河一景"/>
			</mu-tabs>
...
<script>
    ...
    export default {
        ...
        mounted(){
            console.log(this.getRivers)
            let that = this;
            let height = window.innerHeight;
            this.$refs.river.style.height = height - 110 + 'px';
            ...
            function getRiverInfo() {
                that.$http.get(GET_RIVER_INFO).then(function (response) {
                    ...
                    // that.rivers = [
                    //     //level:1-3,不黑不臭，重度黑臭。
                    //     {name: '茅洲河', level: 2,manager:'黄敏'},
                    //     {name: '排涝河', level: 3,manager:'姚任'},
                    //     {name: '新圳河', level: 1,manager:'郑新强'},
                    //     {name: '双界河', level: 3,manager:'吴晶'},
                    //     {name: '西乡河', level: 1,manager:'黄盛华'},
                    //     {name: '福永河', level: 3,manager:'邓桂洪'},
                    //     {name: '沙福河', level: 3,manager:'双德会'},
                    //     {name: '沙井河', level: 3,manager:'张志彪'},
                    //     {name: '罗田水', level: 3,manager:'张现军'},
                    //     {name: '石岩河', level: 2,manager:'郭子平'}
                    // ];
                })
            }
