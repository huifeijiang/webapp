    ...
    <div>
        ...
        <div class="list-conn" v-show="show">
            <div class="li-list f_clear">
                ...
                <div class="num-ri f_ri">{{canting + car + totla}}</div>
            </div>
            <div class="ma10">
                <div class="li-list f_clear">
                    <div class="name-le f_le">巡查企业数-小散乱污</div>
                    <div class="num-ri f_ri">{{totla}}</div>
                </div>
                <div class="li-list f_clear">
                    <div class="name-le f_le">巡查企业数-餐饮行业</div>
                    <div class="num-ri f_ri">{{canting}}</div>
                </div>
                <div class="li-list f_clear">
                    <div class="name-le f_le">巡查企业数-汽修洗车</div>
                    <div class="num-ri f_ri">{{car}}</div>
                </div>
            </div>
            <div class="li-list f_clear">
...
export default {
    ...
    methods: {
        ...
        getTongji(startTime, endTime) {
            //statistics/statisticsLittleDirtByTwo
            this.$http.get(GET_STATICS_NUM + "?startTime=" + startTime + '&endTime=' + endTime).then(res => {
            ...
            this.$http.get(GET_CAR + "?startTime=" + startTime + '&endTime=' + endTime).then(res => {
                let ress = res.body.data;
                this.car = ress.total;
            ...
            this.$http.get(GET_CATING + "?startTime=" + startTime + '&endTime=' + endTime).then(res => {
                let ress = res.body.data;
                this.canting = ress.total;
        ...
        getMeans(startTime, endTime) {
            this.$http.get(GET_EVENT_MEAN + "?startTime=" + startTime + '&endTime=' + endTime).then(res => {
    ...
    mounted() {
        ...
        this.getTongji(this.startTime, this.endTime);
        this.getMeans(this.startTime, this.endTime);
        eventBus.bus.on(eventBus.TONGJISHUJU, (start, end) => {
            this.getTongji(start, end);
            this.getMeans(start, end);
        })
    }
}
</script>
