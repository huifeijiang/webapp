<template>
  ...
  <div class='event'>
    ...
    <cateringdeatil :open="open" :dataTemp="dataTemp" @closeDetail="closeDrawer" ref="deatil"></cateringdeatil>
  </div>
</template>
...
<script type='es6'>
  ...
  import cateringdeatil from './cateringdeatil.vue';
  ...
  export default {
    ...
    methods: {
        ...
        switch (item.name) {
          case '上报事件':
            if (this.$route.params.isadmin) {
              const url1 = GET_CATERING_LIST + '?size=50&page=0';
              this.getEventList(url1, this.wait);
            } else {
              const url1 = GET_CATERING_LIST + '?size=50&userId=' + this.getUserId + '&page=' + this.page;
              this.getEventList(url1, this.wait);
            }
            break;
          case '核查事件':
      ...
      rowClick(data) {
        this.open = true;
        let url = GET_CATERING_DETAIL + "/v1/caterings/" + data.id + '/details';
      ...
      loadMore() {
        ...
        this.page += 1
        if (this.$route.params.isadmin) {
          const url1 = GET_CATERING_LIST + '?size=50&page=' + this.page;
          this.getEventList(url1, this.wait);
        } else {
          const url1 = GET_CATERING_LIST + '?size=50&userId=' + this.getUserId + '&page=' + this.page;
          this.getEventList(url1, this.wait);
