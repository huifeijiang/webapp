<template>
  ...
  <div class='event'>
    ...
    <!--<mu-tabs :value="activeTab" @change="handleTabChange" class="zin74" :class="this.$route.params.isadmin ?'zin743':'zin74'">
            <mu-tab value="tab1" @active="" :title="'上报事件 '+(wait.total || '0')" />
            <mu-tab value="tab2" @active="" :title="'核查事件 '+(finish.total || '0')" />
          </mu-tabs>-->
    <section ref="table" :class="this.$route.params.isadmin ?'pos':'posaaa'">
      ...
      <div class="list-wrap" ref="scroller">
        <div class="list-content">
          <div class="list" v-for="(item,index) in wait.data" @click="rowClick(item)">
            <div class="list-level" :style="getColor(item.level)">{{item.level}}</div>
            <div class="list-detail">
              <div class="detail-left">
                <p class="p1">{{item.reporterName}}</p>
    ...
    <lamp-detail :open="open" :dataTemp="dataTemp" @closeDetail="closeDrawer" ref="deatil"></lamp-detail>
  </div>
</template>
...
<script type='es6'>
  ...
  import lampDetail from './lampblockdeatil.vue';
  ...
  export default {
    ...
    methods: {
        ...
        switch (item.name) {
          case '上报事件':
            if (this.$route.params.isadmin) {
              const url1 = GET_LAMP_EVENT + '?size=50&page=0';
              this.getEventList(url1, this.wait);
            } else {
              const url1 = GET_LAMP_EVENT + '?size=50&userId=' + this.getUserId + '&page=' + this.page;
              this.getEventList(url1, this.wait);
            }
            break;
          case '核查事件':
      ...
      rowClick(data) {
        this.open = true;
        let url = GET_LAMP_DETAIL + "/v1/lampblack/" + data.id + '/details';
      ...
      loadMore() {
        ...
        this.page += 1
        if (this.$route.params.isadmin) {
          const url1 = GET_LAMP_EVENT + '?size=50&page=' + this.page;
          this.getEventList(url1, this.wait);
        } else {
          const url1 = GET_LAMP_EVENT + '?size=50&userId=' + this.getUserId + '&page=' + this.page;
          this.getEventList(url1, this.wait);
    ...
    mounted() {
      ...
      if (this.$route.params.isadmin) {
        const url1 = GET_LAMP_EVENT + '?size=50&page=0';
        const url2 = GET_LAMP_EVENT + '?size=2000&status=2';
        this.getEventList(url1, this.wait);
        //this.getEventList(url2, this.finish);
      } else {
        const url1 = GET_LAMP_EVENT + '?size=50&userId=' + this.getUserId + '&page=' + this.page;
        this.getEventList(url1, this.wait);
      }
