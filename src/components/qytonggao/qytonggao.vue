<template>
	<div class="newss">
		<head-dom type="back" @back="backToMap">通告</head-dom>
		...
		<div class="three-img">
	  	...
	  	<mu-drawer class="news-alert" right :open="open" @close="toggle()">
		    <div class="alert-hed">
		    	<div class="alert-lt" @click="open=false">
		    		<img src="static/images/ic-return.png" alt="">
		    	</div>
				<span class="alert-te" >通告详情</span>
				<span class="alert-rt">隐藏</span>
		    </div>
		...
		methods:{
			...
			toggle(item){
            	...
            	if(item.content ==  undefined || item.content == ''){
              		this.$http.get(GET_TONGGAO_DETAIL +  item.id ).then(function (reponse) {
			...
			handleTabChange(val) {
	      	...
	      	queryList(value) {
		        let that = this;
		        this.$http.get(GET_TONGGAO + '?size=10&page='+this.page).then(function (reponse) {
			        if(reponse.body.code == 200){
			        	that.page += 1;
	      	...
	      	loadMore(){
	      		...
	      		this.loading = true;
	      		this.$http.get(GET_TONGGAO + "?size=10&page="+this.page).then( (reponse) => {
	      			let content  = reponse.body.data.content;
