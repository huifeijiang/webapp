...
import {LayerData}  from '../HomeMap/LayerData';
        ...
        (Map,
         ...
         SpatialReference
            ...
            for(let url of layerNameMap.keys())
            {
                ...
                let wmtsId = ''
                if(layerType == layerTypeEnum.WMTSLayer)
                {
                  wmtsId = layerConfigs[0].LAYERID

                }

               let layer = layerManager.getAgsLayerFromMapWithUrl(map,url,layerType,wmtsId);
                ...
                else if(layerType == layerTypeEnum.WMTSLayer)
                {
                  if(isShow)
                  {
                    console.log('showmtsLayer')
                    showWMTSLayerFun(map,layer,url,priority,layerConfigs[0]);
                  }
                  else
                  {
                    hideWMTSLayerFun(layer);
                  }
                }
...
function addLayerByPriority(map,layer,priority)
{
    ...
    for(let i =  arrIds.length - 1; i >=0 ; i--){
        ...
        if(i == 0)
        {
          layer['priority'] = priority;
          map.addLayer(layer);
          map.on('layer-add-result',function addHandle(addLayer)
          {
            map.reorderLayer(addLayer,2);
          });
          return;
        }
    }
...
//加载wtms 这里固定加载 后面会改参数
function showWMTSLayerFun(map,layer,url,priority,layerConfig){
  ...
  esriLoader.dojoRequire(['esri/map',
    ...
    (Map,
     ...
     SpatialReference
      ...
      if(layer == null)
      {
        let options = layerManager.createWMTSOption(layerConfig.DISPLAYLAYERNAME);
        let imageLayer = new esri.layers.WMTSLayer(url,options);
        imageLayer['wmtsId'] = layerConfig.LAYERID;
...
function hideWMTSLayerFun(layer){
...
layerManager.getAgsLayerFromMapWithUrl = function(map,url,mapServiceType = '',wmtsId = '')
{
    let agsLyr = null;
    let arrIds = map.layerIds;
    for(let layerId of arrIds)
    {
        ...
        else
        {
            ...
            {
                if(getLayerType(item) == layerTypeEnum.WMTSLayer)
                {
                  if(item.wmtsId == wmtsId && item.url == url)
                  {
                    agsLyr = item;
                  }
                }
...
function getLayerType(item)
{
    ...
    else if(Object.getPrototypeOf(item) == esri.layers.WMTSLayer.prototype)
    {
      return layerTypeEnum.WMTSLayer;
    }
...
layerManager.createWMTSOption = function (layerName) {
  ...
  esriLoader.dojoRequire(['esri/map',
    ...
    (Map,
     ...
     SpatialReference
      ...
      let wmtsInfo = layerConfig.getWMTSInfo();

      let tileInfo1 = new TileInfo({
      ...
      let tileExtent1 = new Extent(wmtsInfo.xmin,wmtsInfo.ymin,wmtsInfo.xmax,wmtsInfo.ymax, new SpatialReference({
        wkid: wmtsInfo.wkid
      }));

      let initialExtent = new Extent(mapConfig.extent.xmin,mapConfig.extent.ymin,mapConfig.extent.xmax, mapConfig.extent.ymax, new SpatialReference({
        wkid: wmtsInfo.wkid
      }));

      let layerInfo1 = new WMTSLayerInfo({
        tileInfo: tileInfo1,
        fullExtent: tileExtent1,
        initialExtent: initialExtent,
        identifier: layerConfig.LAYERID,
