<template>
  <div>
    ...
    <mu-float-button v-show="iszhifa" @click="toZx" class="demo-float-button law-btn">专项执法
    </mu-float-button>
    <mu-float-button v-show="iszhifa" @click="zhifa" class="demo-float-button law-btnDD">日常执法
    </mu-float-button>
    ...
    iszhifa: {
      get() {
        if (this.getPortalUIShow() == true) {
          ...
          if (mapUIAccessTool.SinoProbezhifa.includes(Number(this.getRole()))) {
            return true;
          }
          else {
            return false;
